...
//// 上层发来的数据缓存（采用循环队列）
# define N 500 // Buffer 大小
struct pkt BufferA[N]; //这里改为缓存封装好的pkt
int endA; // 上层共送来了 endA 个msg，endA 有可能比N大（应该换个名字）

//// 状态变量
# define n 5 // Go back N steps
int baseA; // 正在传送这个 也用此当作ack序号
int nextseqnum; // 下一个序号
int count; // 快速重传计数器，收到三个执行快速重传
...
{
    // step1: 转换msg，存取pkt，放入BufferA
    // 此处如果传入的数据太快而N太小会导致数据覆盖（方法：舍弃）
    if((endA-baseA)<N)
    {
        msg_to_pkt(&message, &BufferA[endA%N], endA, -1);
        endA++;
        if(Pt)
    ...
    // step2: 如果还能接着发，满足 n steps，就继续发
    // 如果之前的msg都发掉了，这里启动继续发送
    while(nextseqnum-baseA<n && nextseqnum<endA)
    {
        tolayer3(0, BufferA[nextseqnum%N]);
        if(baseA == nextseqnum)
            starttimer(0, 20.0);
        if(Pt)
            printf("A snd pkt %d by layer3\n", BufferA[nextseqnum%N].seqnum);
        nextseqnum++;
    }
}
...
void A_timerinterrupt(void)
{
    ...
    tolayer3(0, BufferA[baseA%N]);
    if(Pt)
        printf("A snd pkt %d by layer3\n", BufferA[baseA%N].seqnum);
    starttimer(0, 20.0);
}
...
void A_input(struct pkt packet)
{
    ...
    if((!IsCorrupt(&packet)) && (packet.acknum>=baseA))
    {
        stoptimer(0);
        if(Pt)
...
// B端
// 下层发来的数据缓存（采用循环队列）
struct pkt BufferB[N]; //这里改为缓存封装好的pkt
int endB; // 上层共送来了endA个msg，endA有可能比N大（应该换个名字）
...
void B_input(struct pkt packet)
{
    if(!IsCorrupt(&packet))
    {
        if((packet.seqnum>=AckB&&packet.seqnum<(AckB+n)))
        {
            BufferB[packet.seqnum%N] = packet;
        }
        if(packet.seqnum==AckB)
        {
            int t = AckB;
            while(BufferB[AckB%N].seqnum>=t && BufferB[AckB%N].seqnum<(t+n))
            {
                tolayer5(1, BufferB[AckB%N].payload);
                AckB++;
            }
            ackpkt.acknum = AckB-1;
            ackpkt.seqnum = SeqB;
            ackpkt.checksum = Checksum(&ackpkt);
            tolayer3(1, ackpkt);
            if(Pt)
    ...
    else
    {
        ...
        {
            printf("****************************************\n");
            printf("B get a corrupted pkt!\n");
            printf("CheckSum %d and Correct is %d\n", packet.checksum, Checksum(&packet));
            printf("****************************************\n");
        }
    }
}
